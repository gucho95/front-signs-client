...
const TabFields = ({ isActiveSlide, item, parentKey, errors, register, control }) => (
  <div className={isActiveSlide ? 'grid gap-y-2' : 'hidden'}>
    {item?.map(({ component: Component, name, rules, ...rest }, childKey) => {
      const fieldName = `items.${parentKey}.${childKey}.${name}`;
      const error = errors?.items?.[parentKey]?.[childKey]?.[name];
...
const ChangableImagesForm = ({ isUpdateMode }) => {
  ...
  const { register, watch, control, setValue, formState, setFocus, getValues } = useFormContext();
  ...
  useEffect(() => {
    ...
    const errorFieldName = `items.${firstErrorTabKey}.${firstErrorFieldKey}.image`;
